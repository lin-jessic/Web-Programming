...
import {
  clearCloudGeneralCommentsByOwner,
  clearCloudUserWorks,
  clearCloudWallByOwner,
  createCloudUser,
  deleteCloudUserWork,
  deleteCloudWallItem,
  findCloudUserByGmail,
  getCloudGeneralComments,
  getCloudUserWorks,
  getCloudWall,
  prepareCloudImage,
  saveCloudGeneralComment,
  saveCloudUserWork,
  saveCloudWallItem,
  updateCloudUser
} from "./firebaseSharedStore.js";
import "./App.css";
...
function isEmbeddedImageString(value) {
  const text = String(value || "").trim();
  return /^data:image\//i.test(text) || (text.length > 500 && /base64,/i.test(text));
}

function safeDisplayName(value, fallback = "Guest") {
  const text = String(value || "").trim();
  if (!text || isEmbeddedImageString(text)) return fallback;
...
function safeCommentText(value) {
  const text = String(value || "").trim();
  if (!text) return "";
  if (isEmbeddedImageString(text)) return "[圖片資料已隱藏]";
  return text;
}

function getStableCommentId(source) {
  if (source.id) return source.id;
  const rawName = source.name || source.nickname || "Guest";
  const rawText = source.text || source.message || source.caption || "";
  const seed = `${source.gmail || "guest"}|${safeDisplayName(rawName)}|${source.createdAt || ""}|${safeCommentText(rawText)}`;
...
function normalizeWallComment(comment) {
  ...
  return {
    ...
    userId: isEmbeddedImageString(source.userId) ? (source.gmail || "guest") : (source.userId || source.gmail || "guest"),
    gmail: isEmbeddedImageString(source.gmail) ? "" : (source.gmail || ""),
    name: safeDisplayName(source.name || source.nickname, "Guest"),
    avatar: isEmbeddedImageString(source.avatar) ? "🌷" : (source.avatar || "🌷"),
    text: safeCommentText(rawText),
    createdAt: isEmbeddedImageString(source.createdAt) ? new Date().toLocaleString() : (source.createdAt || new Date().toLocaleString()),
...
function AuthPage({ onLogin }) {
  ...
  const register = async () => {
    ...
    if (!cleanGmail.includes("@gmail.com")) {
    ...
    try {
      const result = await createCloudUser(newUser);

      if (result.exists) {
        alert("這個 Gmail 已經註冊過了，可以直接登入。換裝置也會找得到同一個帳號。");
        return;
      }
      ...
      console.warn("Cloud registration failed. Falling back to local registration.", error);
    }

    const users = getList(STORAGE_KEYS.users);
    const existed = users.find((user) => user.gmail === cleanGmail);

    if (existed) {
      alert("這個 Gmail 已經在這台裝置註冊過了，可以直接登入。");
      return;
    }
  ...
  const login = async () => {
    ...
    try {
      const cloudUser = await findCloudUserByGmail(cleanGmail);
      if (cloudUser) {
        localStorage.setItem(STORAGE_KEYS.currentUser, JSON.stringify(cloudUser));
        onLogin(cloudUser);
        return;
      }
    } catch (error) {
      console.warn("Cloud login failed. Falling back to local login.", error);
    }

    const users = getList(STORAGE_KEYS.users);
    const user = users.find((item) => item.gmail === cleanGmail);

    if (!user) {
      alert("找不到這個帳號，請先註冊。若你已經在別台裝置註冊，請確認 Firebase Firestore 已啟用並允許讀寫。");
      return;
    }
    ...
    <div className="auth-page">
      <div className="auth-card">
        ...
        <p className="auth-note">
          Accounts and Community Wall sync through Firebase Firestore after deployment. If Firestore is unavailable, this page keeps a local demo fallback.
        </p>
      </div>
    </div>
  );
...
function MyStoragePage({ refreshKey, currentUser, onShareToWall }) {
  ...
  const loadStorage = async () => {
    const sameOwner = (item) => !item.ownerGmail || item.ownerGmail === currentUser.gmail;
    const mergeById = (localItems, cloudItems) => {
      ...
      return Array.from(map.values()).sort(
        (a, b) => (b.createdAtMs || Number(String(b.id).split("-")[0]) || 0) - (a.createdAtMs || Number(String(a.id).split("-")[0]) || 0)
      );
    };

    const localPostcards = await attachImages(getList(STORAGE_KEYS.postcards).filter(sameOwner));
    const localPhotoBooths = await attachImages(getList(STORAGE_KEYS.photoBooths).filter(sameOwner));
    const localScrapbooks = await attachImages(getList(STORAGE_KEYS.scrapbooks).filter(sameOwner));

    let cloudWorks = null;
    try {
      cloudWorks = await getCloudUserWorks(currentUser.gmail);
    } catch (error) {
      console.warn("Cloud storage load failed.", error);
    }

    const cloudPostcards = (cloudWorks || []).filter((item) => item.type === "postcard");
    const cloudPhotoBooths = (cloudWorks || []).filter((item) => item.type === "photobooth");
    const cloudScrapbooks = (cloudWorks || []).filter((item) => item.type === "scrapbook");

    setPostcards(mergeById(localPostcards, cloudPostcards));
    setPhotoBooths(mergeById(localPhotoBooths, cloudPhotoBooths));
    setScrapbooks(mergeById(localScrapbooks, cloudScrapbooks));
  };
  ...
  const deleteItem = async (type, id) => {
    ...
    try { await deleteCloudUserWork(currentUser.gmail, id); } catch (error) { console.warn("Cloud delete failed.", error); }
  ...
  const clearType = async (type) => {
    ...
    try { await clearCloudUserWorks(currentUser.gmail, type); } catch (error) { console.warn("Cloud clear failed.", error); }
...
function CommunityWallPage({ refreshKey, currentUser }) {
  ...
  const loadWall = async () => {
    ...
    const localGallery = await attachImages(normalizeWallGallery(getList(STORAGE_KEYS.gallery)));
    let cloudGallery = null;
    try {
      cloudGallery = await getCloudWall();
    } catch (error) {
      console.warn("Cloud wall load failed.", error);
    }
    const nextGallery = normalizeWallGallery(cloudGallery ? mergeById([...localGallery, ...cloudGallery]) : localGallery);
    setGallery(nextGallery);
    saveList(STORAGE_KEYS.gallery, stripImages(nextGallery), MAX_GALLERY);

    const localComments = getList(STORAGE_KEYS.comments).map(normalizeWallComment);
    let cloudComments = null;
    try {
      cloudComments = await getCloudGeneralComments();
    } catch (error) {
      console.warn("Cloud comments load failed.", error);
    }
    const nextComments = mergeById([...(localComments || []), ...((cloudComments || []).map(normalizeWallComment))]).slice(0, MAX_COMMENTS);
    setComments(nextComments);
    saveList(STORAGE_KEYS.comments, nextComments, MAX_COMMENTS);

    const localPostcards = await attachImages(getList(STORAGE_KEYS.postcards).filter(
      (item) => !item.ownerGmail || item.ownerGmail === currentUser.gmail
    ));

    const localPhotoBooths = await attachImages(getList(STORAGE_KEYS.photoBooths).filter(
      (item) => !item.ownerGmail || item.ownerGmail === currentUser.gmail
    ));
    ...
    try {
      ...
      console.warn("Cloud user works load failed.", error);
    }

    setSavedPostcards(mergeById([...localPostcards, ...((cloudWorks || []).filter((item) => item.type === "postcard"))]));
    setSavedPhotoBooths(mergeById([...localPhotoBooths, ...((cloudWorks || []).filter((item) => item.type === "photobooth"))]));
  };
  ...
  const postToWall = async (image, caption) => {
    ...
    const cloudImage = await prepareCloudImage(image);
    ...
    try { await saveCloudWallItem(item); } catch (error) { console.warn("Cloud wall post failed.", error); }
  ...
  const toggleLike = async (postId) => {
    ...
    if (updatedPost) {
      try { await saveCloudWallItem(updatedPost); } catch (error) { console.warn("Cloud like update failed.", error); }
    }
  };
  ...
  const toggleFavorite = async (postId) => {
    ...
    saveList(STORAGE_KEYS.gallery, stripImages(next), MAX_GALLERY);
    if (updatedPost) {
      try { await saveCloudWallItem(updatedPost); } catch (error) { console.warn("Cloud favorite update failed.", error); }
    }
  };
  ...
  const deleteWallPost = async (postId) => {
    ...
    try { await deleteCloudWallItem(postId); } catch (error) { console.warn("Cloud wall delete failed.", error); }
  ...
  const editWallPost = async (postId, newCaption) => {
    ...
    const next = gallery.map((item) => {
      if (item.id !== postId) return item;
      updatedPost = { ...item, caption: newCaption };
      return updatedPost;
    });
    ...
    saveList(STORAGE_KEYS.gallery, stripImages(next), MAX_GALLERY);
    if (updatedPost) {
      try { await saveCloudWallItem(updatedPost); } catch (error) { console.warn("Cloud caption update failed.", error); }
    }
  };
  ...
  const addWallComment = async (postId, text, parentCommentId = null) => {
    ...
    const normalizedGallery = normalizeWallGallery(gallery);
    let updatedPost = null;
    ...
    const next = normalizedGallery.map((item) => {
      ...
      if (parentCommentId) {
        ...
        updatedPost = { ...item, comments: result.comments };
        return updatedPost;
      }
    ...
    if (updatedPost) {
      setGallery(next);
      saveList(STORAGE_KEYS.gallery, stripImages(next), MAX_GALLERY);
      try { await saveCloudWallItem(updatedPost); } catch (error) { console.warn("Cloud wall comment failed.", error); }
    }
  };
  ...
  const addGeneralComment = async () => {
    ...
    try { await saveCloudGeneralComment(item); } catch (error) { console.warn("Cloud general comment failed.", error); }
  ...
  const addGeneralReply = async (commentId, text) => {
    ...
    saveList(STORAGE_KEYS.comments, next, MAX_COMMENTS);
    if (updatedComment) {
      try { await saveCloudGeneralComment(updatedComment); } catch (error) { console.warn("Cloud general reply failed.", error); }
    }
  ...
  const clearWall = async () => {
    if (!window.confirm("只清除你自己發布到 Community Wall 的作品嗎？")) return;
    const current = getList(STORAGE_KEYS.gallery);
    const mine = current.filter((item) => item.gmail === currentUser.gmail);
    await Promise.all(mine.map((item) => deleteImageFromDB(item.imageKey)));
    try { await clearCloudWallByOwner(currentUser.gmail); } catch (error) { console.warn("Cloud wall clear failed.", error); }
    const next = gallery.filter((item) => item.gmail !== currentUser.gmail);
    saveList(STORAGE_KEYS.gallery, stripImages(next), MAX_GALLERY);
    setGallery(next);
  };

  const clearComments = async () => {
    if (!window.confirm("只清除你自己的留言嗎？")) return;
    try { await clearCloudGeneralCommentsByOwner(currentUser.gmail); } catch (error) { console.warn("Cloud comments clear failed.", error); }
    const next = comments.filter((item) => item.gmail !== currentUser.gmail);
    saveList(STORAGE_KEYS.comments, next, MAX_COMMENTS);
    setComments(next);
  };
...
function App() {
  ...
  const updateAvatarData = async (imageDataUrl) => {
    try {
      ...
      const cloudAvatar = await prepareCloudImage(imageDataUrl, 500, 0.72);
      ...
      const users = getList(STORAGE_KEYS.users);
      const nextUsers = users.map(u => u.id === currentUser.id || u.gmail === currentUser.gmail ? { ...u, avatarKey: avatarKey, avatar: cloudAvatar } : u);
      localStorage.setItem(STORAGE_KEYS.users, JSON.stringify(nextUsers));
      try { await updateCloudUser(updatedUser); } catch (error) { console.warn("Cloud avatar update failed.", error); }
  ...
  const saveArtwork = async (type, item) => {
    try {
      ...
      const cloudImage = item.image ? await prepareCloudImage(item.image) : null;
      const itemWithOwner = {
      ...
      const cloudItem = { ...itemWithOwner, image: cloudImage || item.image };
      ...
      if (ok && currentUser?.gmail) {
        try { await saveCloudUserWork(currentUser.gmail, cloudItem); } catch (error) { console.warn("Cloud artwork save failed.", error); }
        setRefreshKey((prev) => prev + 1);
      }
      return ok;
    } catch (error) {
  ...
  const shareToWall = async (item) => {
    ...
    const cloudImage = await prepareCloudImage(imageSource);
    ...
    const wallItem = {
      ...
      createdAt: new Date().toLocaleString(), createdAtMs: Date.now(), image: cloudImage, likes: [], comments: [], favorites: []
    };

    const next = [wallItem, ...current];
    const ok = saveList(STORAGE_KEYS.gallery, stripImages(next), MAX_GALLERY);
    try { await saveCloudWallItem(wallItem); } catch (error) { console.warn("Cloud share to wall failed.", error); }
...
/* Final Community Board long-text overflow fix */
const communityCommentNoOverflowFixStyle = document.createElement("style");
communityCommentNoOverflowFixStyle.textContent = `
  .community-layout .comment-board-card,
  .community-layout .comment-list,
  .community-layout .comment-item,
  .community-layout .comment-item *,
  .community-layout .wall-comment-list,
  .community-layout .wall-comment-item,
  .community-layout .wall-comment-item * {
  ...
  .community-layout .comment-item > div {
    max-width: 100% !important;
    min-width: 0 !important;
    overflow: hidden !important;
  }

  .community-layout .comment-item strong,
  .community-layout .comment-item small,
  .community-layout .comment-item p,
  .community-layout .wall-comment-item strong,
  .community-layout .wall-comment-item small,
  .community-layout .wall-comment-item p {
    max-width: 100% !important;
  ...
  .community-layout .comment-form textarea,
  .community-layout .comment-form button {
    width: 100% !important;
    max-width: 100% !important;
  }
`;
document.head.appendChild(communityCommentNoOverflowFixStyle);
